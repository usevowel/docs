...
![RAG Generated Files](https://blog-assets.vowel.to/assets/voweldocs-rag-files.png)

The build process creates a complete RAG knowledge base including:
- `docs.json` — chunked documentation content with metadata
- `embeddings.json` — vector embeddings for semantic search
- `stats.json` — index statistics for the VectorDB

These files are served as static assets and loaded into Haven's local VectorDB on page initialization.
...
![RAG Debug Chat](/images/rag-chat.png)

## Agent Skills

- **`haven-local-rag`** (`.agents/skills/haven-local-rag/`) - Complete guide to Haven VectorDB, browser embeddings, and local RAG pipelines
