...
export async function getIndexedDocuments(): Promise<DebugDocument[]> {
  try {
    // Check if RAG can be initialized (Haven must load first)
    updateStatusMessage('Loading RAG system...', 'loading');
      ...
      if (statusText?.includes('Error')) {
        treeContainer.innerHTML = `
          <div class="rag-debug-empty">
            ...
            <button class="rag-debug-btn" style="margin-top: 12px;" id="rag-debug-retry-haven">
              ${ICONS.refresh} Retry Loading
            </button>
          </div>
        `;
        treeContainer.querySelector('#rag-debug-retry-haven')?.addEventListener('click', () => {
