...
export async function sendChatMessage(query: string): Promise<void> {
  ...
  try {
    await addChatMessage('system', 'Loading Haven and searching...');
    ...
    messages?.forEach(m => {
      if (m.textContent?.includes('Loading Haven')) {
        m.remove();
      }
    });
    ...
    await addChatMessage('system', 'Tip: If Haven keeps failing to load, check your internet connection and try refreshing the page.');
  }
}
