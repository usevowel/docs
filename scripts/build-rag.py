...
def save_build_state(state: BuildState, state_path: Path) -> None:
    ...
    try:
        with open(state_path, "w", encoding="utf-8") as f:
            yaml.dump(state.to_dict(), f, default_flow_style=False, allow_unicode=True, sort_keys=False)
    except Exception as e:
...
def is_chunk_changed(chunk_id: str, chunk_text: str, file_state: Optional[FileState]) -> tuple[bool, Optional[List[float]]]:
    """
...
def get_latest_llama_version() -> str:
    ...
    try:
        print("🔍 Checking for latest llama.cpp version...")
        ...
        print(f"   ✓ Latest version: {tag_name} (build {version})")
        return version
    except Exception as e:
        print(f"   ⚠️  Failed to fetch latest version: {e}")
        print("   Falling back to pinned version: b8671")
        return "8671"
...
class LlamaServerManager:
    ...
    def download_binary(self) -> None:
        ...
        print(f"📦 Downloading llama-server ({version_tag})...")
        ...
        print(f"   ✓ llama-server ready at {self.server_binary}")
    ...
    def download_model(self) -> None:
        ...
        print(f"📦 Downloading embedding model ({self.config.model_file})...")
        ...
        print(f"   ✓ Model ready at {self.config.model_path}")
    ...
    def start(self) -> None:
        ...
        print(f"🚀 Starting llama-server on {self.config.server_host}:{self.config.server_port}...")
        ...
        cmd = [
            ...
            "-np", str(self.config.n_parallel),  # Parallel slots
        ...
        print("   ✓ llama-server ready (Vulkan GPU acceleration active)")
...
def load_all_documents(docs_dir: Path) -> List[RawDocument]:
    ...
    EXCLUDE_DIRS = {
        '.agents', 'agents', 'node_modules', '.vitepress', '.vite', '.git',
    ...
    print(f"   ✓ Loaded {len(documents)} documents ({len(md_files)} files scanned)")
    return documents
...
def process_documents_incremental(
    ...
    print("✂️  Chunking documents (incremental)...")
    ...
    for doc in tqdm(documents, desc="Chunking"):
        ...
        full_file_path = config.docs_dir / doc.path.replace("/", os.sep)
        ...
        for idx, chunk_text in enumerate(chunks):
            ...
            if not file_changed:
                ...
                has_changed, cached_embedding = is_chunk_changed(chunk_id, contextualized_text, prev_file_state)
    ...
    print(f"   ✓ Created {total_chunks} chunks ({skipped_chunks} skipped, {len(chunks_to_embed)} new/modified)")
...
def save_index(chunks: List[DocumentChunk], output_path: Path, config: Optional[Config] = None) -> None:
    ...
    # Note: Haven VectorDB expects 'vector' not 'embedding' field
    # Convert embeddings to lists for YAML serialization
    index_data = {
        "version": "1.0.0",
        "model": config.model_repo,
        "model_file": config.model_file,
        ...
        "chunks": [
            {
                "id": chunk.id,
                "text": chunk.text,
                "vector": chunk.embedding.tolist() if hasattr(chunk.embedding, 'tolist') else list(chunk.embedding),  # Convert to list for YAML
    ...
    print(f"   ✓ Saved {len(chunks)} chunks ({file_size / 1024 / 1024:.2f} MB)")


def save_documents_index(documents: List[RawDocument], output_path: Path, config: Optional[Config] = None) -> None:
    """
    ...
    print(f"💾 Saving documents index to {docs_output_path}...")
    ...
    with open(docs_output_path, "w", encoding="utf-8") as f:
        yaml.dump(documents_data, f, default_flow_style=False, allow_unicode=True, sort_keys=False)

    file_size = docs_output_path.stat().st_size
    print(f"   ✓ Saved {len(documents)} documents ({file_size / 1024 / 1024:.2f} MB)")


def load_existing_chunks(output_path: Path) -> List[DocumentChunk]:
    """Load existing chunks from the output file if it exists."""
    ...
    try:
        ...
        # Support both old format (chunks with 'embedding') and new format (chunks with 'vector')
        chunks_data = data.get("chunks", [])
        return [
            DocumentChunk(
                id=chunk["id"],
                text=chunk["text"],
                # Use 'vector' if available (new format), otherwise fall back to 'embedding' (old format)
                embedding=chunk.get("vector") or chunk.get("embedding"),
                metadata=chunk["metadata"],
            )
...
def build_rag_index(config: Optional[Config] = None) -> None:
    ...
    config = config or get_config()
    print("=" * 60)
    print("🚀 RAG Prebuild Pipeline - llama.cpp Vulkan Edition")
    print("   (Incremental builds enabled)")
    print("=" * 60)

    # 1. Load previous build state for incremental processing
    previous_state = load_build_state(config.state_path)
    if previous_state:
        print(f"📋 Loaded previous build state ({len(previous_state.files)} files tracked)")
    else:
        print("📋 No previous build state found - will perform full rebuild")
    ...
    chunks_to_embed, new_state, skipped_count, total_chunks = process_documents_incremental(
        documents, previous_state, config
    )
    ...
    if skipped_count > 0 and previous_state:
        existing_chunks = load_existing_chunks(config.output_path)
        print(f"📂 Loaded {len(existing_chunks)} existing chunks from previous build")

    # 5. Generate embeddings for new/changed chunks (if any)
    new_chunks: List[DocumentChunk] = []
    if chunks_to_embed:
        print(f"⚡ Generating embeddings for {len(chunks_to_embed)} new/modified chunks...")
        try:
        ...
        print("⚡ No new embeddings needed - all chunks up to date")
    ...
    all_chunks = [c for c in all_chunks if c.metadata.get("path", "") in current_file_paths]

    # 8. Update state with new embeddings
    final_state = finalize_state_with_embeddings(new_state, new_chunks)

    # 9. Save the updated state for next incremental build
    save_build_state(final_state, config.state_path)
    print(f"💾 Saved build state ({len(final_state.files)} files)")

    # 10. Save the final index
    save_index(all_chunks, config.output_path, config)

    # 11. Save the documents index for hierarchical retrieval
    save_documents_index(documents, config.output_path, config)

    print("=" * 60)
    print("✅ Build complete!")
    print(f"   Chunk index: {config.output_path}")
    print(f"   Document index: {config.output_path.parent / 'rag-documents.yml'}")
    print(f"   Total chunks: {len(all_chunks)}")
    print(f"   New/modified: {len(new_chunks)}")
    print(f"   Skipped (cached): {skipped_count}")
    print(f"   Dimensions: {config.dimensions}")
    print(f"   GPU: Vulkan acceleration active")
    print("=" * 60)
