...
export function createFloatingButton(): HTMLElement {
  ...
  btn.title = 'Open Turso Browser RAG Debug Tool';
  btn.setAttribute('aria-label', 'Open Turso Browser RAG Debug Tool');
...
export function updateFABProgress(progress: number, isLoading: boolean): void {
  ...
  if (isLoading) {
    ...
    btn.title = `Loading Turso Browser RAG... ${Math.round(progress)}%`;
  } else {
...
function setFABEnabled(enabled: boolean): void {
  const btn = document.getElementById('rag-debug-fab') as HTMLButtonElement | null;
  if (!btn) return;
...
export function initializeRAGDebug(): void {
  ...
  getPrebuiltRAG().then(({ subscribeToInitializationState }) => {
    subscribeToInitializationState((initState) => {
      ...
      // When initialization completes or errors, update status to enable/disable button
      if (initState.stage === 'complete' || initState.stage === 'error') {
        void updateStatus();
      }
    });
  });
  ...
  if (!autoInitStarted) {
    ...
    getLocalRAG().then((localRAG: PrebuiltRAG) => {
      localRAG.initialize((progress: InitializationProgress) => {
        ...
        updateFABProgress(100, false);
        // Update button state to enable it now that RAG is ready
        void updateStatus();
  ...
  console.log('[rag-debug] RAG debug tool initialized. Click the Turso icon in the bottom left to open.');
}
